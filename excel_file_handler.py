...
class Excel_File_Handler:
		...
		def calculate_and_generate_bill(self):
				...
				if self.customer_sheet.max_row > 1:
					...
					self.list_customers()
					print()
					self.customer_id = input('Enter Customer ID to Generate the bill: ')
					...
					for c_row in range(1, self.customer_sheet.max_row + 1):
						if  self.customer_id in self.customer_sheet['A{index}'.format(index=c_row)].value:
							self.customer_name = self.customer_sheet['B{index}'.format(index=c_row)].value
							self.customer_address = self.customer_sheet['C{index}'.format(index=c_row)].value
							self.customer_check_in_date = self.customer_sheet['D{index}'.format(index=c_row)].value
							self.customer_room_id = self.customer_sheet['F{index}'.format(index=c_row)].value
							self.customer_food_id = self.customer_sheet['G{index}'.format(index=c_row)].value
							break
					...
					for r_row in range(1, self.room_sheet.max_row + 1):
						...
						if self.customer_room_id in self.room_sheet['A{index}'.format(index=r_row)].value:
							self.customer_room_no = self.room_sheet['B{index}'.format(index=r_row)].value
							self.customer_room_price = self.room_sheet['C{index}'.format(index=r_row)].value
							self.room_sheet['D{index}'.format(index=r_row)].value = 'No'
							break
					...
					for f_row in range(1, self.food_sheet.max_row + 1):
						if self.customer_food_id in self.food_sheet['A{index}'.format(index=f_row)].value:	
							self.customer_food_price = self.food_sheet['C{index}'.format(index=f_row)].value
							break



					self.pdf_bill_generator(customer_id=self.customer_id, customer_name=self.customer_name,customer_address=self.customer_address, customer_check_in_date=self.customer_check_in_date, customer_check_out_date=self.customer_check_out_date, customer_room_no=self.customer_room_no, customer_room_rent=self.customer_room_price, customer_food_purchased=self.customer_food_price)
		...
		def pdf_bill_generator(self, customer_id, customer_name, customer_address, customer_check_in_date, customer_check_out_date, customer_room_no, customer_room_rent, customer_food_purchased):
			...
			self.total_to_pay = int(int(customer_room_rent) + int(customer_food_purchased) + 100)

			self.pdf.set_font("Arial", size=12)
			self.pdf.cell(200, 10, txt="***********HOTEL BILL************", ln=1, align="C")
			self.pdf.cell(200, 10, txt="Customer ID: {c_id}".format(c_id=customer_id), ln=1, align="C")
			self.pdf.cell(200, 10, txt="Customer Name: {c_name}".format(c_name=customer_name), ln=1, align="C")
			self.pdf.cell(200, 10, txt="Customer Address: {c_add}".format(c_add=customer_address), ln=1, align="C")
			self.pdf.cell(200, 10, txt="Customer Check In Date: {c_ch_in_date}".format(c_ch_in_date=customer_check_in_date), ln=1, align="C")
			self.pdf.cell(200, 10, txt="Customer Check Out Date: {c_ch_out_date}".format(c_ch_out_date=customer_check_out_date), ln=1, align="C")
			self.pdf.cell(200, 10, txt="Room No: {c_room_no}".format(c_room_no=customer_room_no), ln=1, align="C")
			self.pdf.cell(200, 10, txt="Room Rent: {c_room_rent}".format(c_room_rent=customer_room_rent), ln=1, align="C")
			self.pdf.cell(200, 10, txt="Food Purchased Bill: {food_purchased}".format(food_purchased=customer_food_purchased), ln=1, align="C")
			self.pdf.cell(200, 10, txt="Food Service Charges: {service_chareges}".format(service_chareges='100'), ln=1, align="C")
			self.pdf.cell(200, 10, txt="Total To Pay: {t_t_p}".format(t_t_p=self.total_to_pay), ln=1, align="C")
